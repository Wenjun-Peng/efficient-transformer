...
class MoEFFN(nn.Module):
    def __init__(self, config):
        ...
        num_expert = 8
        num_act = 4
        self.MoELayer = MoE(input_size=config.hidden_size, output_size=config.hidden_size, hidden_size=config.hidden_size//num_expert, num_experts=num_expert, k=num_act)
...
class FastformerEncoder(nn.Module):
    def __init__(self, config, is_moe=False, pooler_count=1):
        super(FastformerEncoder, self).__init__()
        self.config = config
        self.encoders = nn.ModuleList([FastformerLayer(config, is_moe) for _ in range(config.num_hidden_layers)])
